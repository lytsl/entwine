...
const SIDEBAR_WIDTH = "13.75rem"; // "20.625rem"
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
	...
	return (
		<button
			...
			className={cn(
				"absolute inset-y-0 z-20 hidden w-3 transition-all ease-linear after:absolute after:inset-y-3 after:start-1/2 after:w-0.5 hover:after:bg-[oklch(0.4551_0.0026_271.31)] group-data-[side=left]:-right-3 group-data-[side=right]:left-0 sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
	return (
		<main
			...
			className={cn(
				"relative h-full min-w-0 flex-1 bg-background md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ms-2 md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ms-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm",
				className,
			)}
			{...props}
		/>
...
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			data-sidebar="header"
			className={cn(
				"mt-2 flex h-10 flex-col justify-center gap-2 px-3.5",
				className,
			)}
			{...props}
		/>
...
function SidebarGroup({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			data-sidebar="group"
			className={cn(
				"relative flex w-full min-w-0 flex-col py-2 ps-3.5 pe-1",
				className,
			)}
			{...props}
		/>
...
function SidebarGroupLabel({
	...
	return useRender({
		...
		props: mergeProps<"div">(
			{
				className: cn(
					"flex h-6 shrink-0 items-center gap-1 rounded-md ps-1 font-medium text-[oklch(0.6698_0.0026_271.4)] text-xs outline-hidden ring-sidebar-ring transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 [&>svg]:size-3 [&>svg]:shrink-0",
					className,
				),
			},
			props,
...
const sidebarMenuButtonVariants = cva(
	"peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md px-1.5 text-start text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0",
	{
		variants: {
			variant: {
			...
			size: {
				...
				icon: "inline-flex size-7 justify-center rounded-lg px-0",
			},
		},
		defaultVariants: {
			variant: "default",
...
function SidebarMenuButton({
	...
	} else if ("keys" in tooltip) {
		tooltip = {
			children: <ShortcutDisplay shortcut={tooltip} />,
		};
	}
...
function SidebarMenuAction({
	...
	return useRender({
		...
		props: mergeProps<"button">(
			{
				className: cn(
					"flex aspect-square w-6 items-center justify-center rounded-md p-0 text-[oklch(0.4551_0.0026_271.31)] outline-hidden ring-sidebar-ring transition-transform hover:bg-sidebar-accent hover:text-[oklch(0.9176_0.0026_271.45)] hover:text-sidebar-accent-foreground focus-visible:ring-2 peer-hover/menu-button:text-sidebar-accent-foreground [&>svg]:size-4 [&>svg]:shrink-0",
					showOnHover &&
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
	return (
		<ul
			...
			className={cn(
				"mx-1.5 flex min-w-0 translate-x-px flex-col gap-0 border-sidebar-border px-1.5 py-0 group-data-[collapsible=icon]:hidden rtl:-translate-x-px",
				className,
			)}
			{...props}
		/>
