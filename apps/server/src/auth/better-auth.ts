...
export const auth = betterAuth({
  ...
  plugins: [
    customSession(async ({ user, session }, ctx) => {
      const orgSlug = ctx.getHeader("x-organization-slug");

      let organization: typeof mainSchema.organization.$inferSelect | null =
        null;
      ...
      if (orgSlug) {
        ...
        const [dbResult] = await db
          ...
          .where(
            and(
              eq(mainSchema.organization.slug, orgSlug),
