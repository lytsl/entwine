			...
			organizationHooks: {
				...
				beforeAddMember: async ({ member, user, organization }) => {
					...
					const similiarUsernameData = await db
						.select({ username: dbSchema.member.username })
						.from(dbSchema.member)
						.where(
							and(
								eq(dbSchema.member.organizationId, organization.id),
								like(dbSchema.member.username, `${emailUsername}%`),
							),
						);
				...
				beforeCreateTeam: async ({ team }) => {
					...
					const similiarSlugData = await db
						.select({ slug: dbSchema.team.slug })
						.from(dbSchema.team)
						.where(
							and(
								eq(dbSchema.team.organizationId, team.organizationId),
								like(dbSchema.team.slug, `${namePrefix}%`),
							),
						);
